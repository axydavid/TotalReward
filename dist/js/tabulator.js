...
;(function (global, factory) {
	...
	Cell.prototype._configureCell = function () {
		...
		if (field) {
			...
			if (field=="img")this.element.setAttribute("class", "mCustomScrollbar");
	...
	Edit.prototype.edit = function (cell, e, forceEdit) {
		...
		function cancel() {
			if (self.currentCell === cell) {
				$(self.currentCell.element.lastChild).fadeOut(300,function(){
					self.cancelEdit();
				});
			...
			if (allowEdit || forceEdit) {
				...
				if (cellEditor !== false) {
					...
					if (cellEditor instanceof Node) {
						...
						$(children[0]).animate({height: children[0].scrollHeight}, 300);
	...
	Edit.prototype.editors = {
		...
		autocomplete: function autocomplete(cell, onRendered, success, cancel, editorParams) {
			...
			function chooseItem(autoselect = false) {
				hideList();
				if (autoselect && displayItems.length > 0 )	
				{
					if(currentItem != false)
					{
						cellEl.firstChild.data = currentItem.value;
						input.value = currentItem.value;
						success(currentItem.value);
					}
					else
					{
						cellEl.firstChild.data = displayItems[0].value;
						input.value = displayItems[0].value;
						success(displayItems[0].value);
					}
				}
				else if (currentItem) {
					if (initialValue !== currentItem.value) {
			...
			input.addEventListener("keyup", function (e) {
				...
				switch (e.keyCode) {
					...
					default:
						...
						if(displayItems.length > 0)setCurrentItem(displayItems[0]);
				}
			});
